...
file_path2 <- "C:/Temp/APR25-Max.xlsx"
...
cons_NonCAWCDF <- cons_NonCAWCD %>% 
  remove_all_na_rows() %>% 
  slice_head(n = -1) %>% # Removes the last row (242 project)
  summarise(across(where(is.numeric), \(x) sum(x, na.rm = TRUE))) # Sums only numeric columns

cons242 <- find_rows_containing_string(cons_NonCAWCD, "242 Wellfield")

current_year <- format(Sys.Date(), "%Y")
column_number <- which(grepl(current_year, colnames(cons_CA)))

# cons242[column_number]
...
# Check if result_dplyr has rows and at least two columns before proceeding
if (nrow(cons_CA_filt) > 0 && ncol(cons_CA_filt) >= 2) {

  # Use apply to iterate over rows. For each row:
  # row[1] will be the value from the first column of result_dplyr
  # row[2] will be the value from the second column of result_dplyr
  bullet_points <- apply(cons_CA_filt, 1, function(row) {
    paste0("  * ", row[1], ": ", row[2], " kaf\n")
  })
  
  # Print each bullet point, separated by a newline
  cat(paste0(bullet_points, collapse = ""))
  
} else if (nrow(result_dplyr) == 0) {
  cat("No data available in `result_dplyr` to create a list.")
} else {
  cat("`result_dplyr` does not have at least two columns as expected.")
}
```
